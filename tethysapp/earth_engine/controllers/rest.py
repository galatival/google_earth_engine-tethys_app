...
def get_time_series(request):
    ...
    if not product or product not in EE_PRODUCTS[platform][sensor]:
        ...
        return HttpResponseBadRequest(f'The "product" parameter is required. Valid sensors for the "{platform} '
    ...
    # if index is still None it is not supported currently
    if index is None:
        return HttpResponseBadRequest(f'Retrieving time series for "{platform} {sensor} {product}" is not supported at this time.')
    ...
    try:
        ...
        return HttpResponseBadRequest(
            f'none The "scale" parameter must be a valid number, but "{scale_str}" was given.'
        )
